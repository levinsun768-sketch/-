"""
训练主入口。
训练配置只依赖 `training/trainer_config.py`。
"""
import datetime
import hashlib
import json
...
# 兼容在 `training/` 目录下直接执行 `python trainer.py`。
...
def _set_seed(seed: int) -> None:
    ...
    # 先采用常用的可复现设置，不强制最严格 deterministic，
    # 避免无必要地影响速度或触发不支持的算子报错。
...
def _configure_logging(log_path: Path) -> logging.Logger:
    ...
    return logging.getLogger("train.trainer")
...
def main():
    ...
    logger.info("Starting training for %s epochs...", cfg.MAX_EPOCHS)
    ...
    for epoch in range(cfg.MAX_EPOCHS):
        ...
        for batch in pbar:
            ...
            if batch_counter % 20 == 0:
                pbar.set_postfix({
                    "Tot": f"{batch_metrics['total_loss']:.4f}",
                    "Enc": f"{batch_metrics['enc_loss']:.4f}",
                    "Dec": f"{batch_metrics['dec_loss']:.4f}",
                })
